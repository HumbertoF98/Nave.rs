...
const Modal = ({ onClose = () => {}, children }) => {
  ...
  // get birthdate
  let age = moment(children.birthdate.slice(0, 10).toString());
  // difference between birthdate and today is the naver's age
  let calc = Math.abs(age.diff(today, "years"));

  // get admission date
  let admissionDate = moment(children.admission_date.slice(0, 10).toString());
    ...
    <ModalContainer>
      <DivContainer>
        ...
        <Content>
          <DivInfo>
            <h1>
              {children.name.length > 20
                ? children.name.substring(0, 20, -3) + "..."
                : children.name}
            </h1>
            <span>
              {children.job_role.length > 30
          ...
          <DivIcons>
            <img
              src={Trash}
              alt="Trash"
              onClick={() => seeModalDelete(children.id)}
            />
            {modalDelete ? (
